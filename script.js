document.addEventListener('DOMContentLoaded', () => {
  ...
  swStart.addEventListener('click',()=>{
    ...
    swStart.classList.toggle('btn-start');
    swStart.classList.toggle('btn-stop');
  ...
  function playAlarmSound(){
    try{
      ...
      for(let i=0;i<6;i++){
        const o=ctx.createOscillator(); o.type='sine'; o.frequency.value=880-i*40; o.connect(gain); o.start(t0+i*0.5); o.stop(t0+i*0.5+0.35);
      }
  ...
  function tick(now){
    ...
    if(nowReal.getSeconds()===0){ alarms.forEach(a=>{ if(!a.enabled) return; if(a.hour===nowReal.getHours()&&a.min===nowReal.getMinutes()){ if(lastTriggered!==keyNow){ lastTriggered=keyNow; saveAll(); try{ if(Notification.permission==='granted'){ new Notification('N Clock',{body:`${t('msg.alarmSound')}: ${String(a.hour).padStart(2,'0')}:${String(a.min).padStart(2,'0')}`}); } }catch(e){} playAlarmSound(); alert(`${t('msg.alarmSound')}: ${String(a.hour).padStart(2,'0
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                }:${String(a.min).padStart(2,'0')}`); } } }); }
